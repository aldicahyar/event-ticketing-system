...
export const Footer = () => {
  ...
  return (
    <footer className="bg-black text-white font-mono border-t border-mono-dark-grey">
      {/* Newsletter Section */}
            ...
            <form onSubmit={handleSubscribe} className="flex gap-2">
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  className="w-full bg-black border border-white text-white px-12 py-4 focus:outline-none focus:border-white/50 placeholder-[#666]"
                  required
                />
              </div>
              <button
                type="submit"
                className="px-6 py-4 bg-white text-black border-2 border-white font-bold uppercase tracking-wide hover:bg-transparent hover:text-white transition-all flex items-center gap-2 whitespace-nowrap"
              >
                {subscribed ? (
                  <>
      ...
      <div className="container mx-auto px-4 md:px-6 py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              <h4 className="font-bold uppercase text-white mb-4">{title}</h4>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link 
                      href={link.href}
                      className="text-xs text-[#CCCCCC] hover:text-white transition-colors uppercase tracking-wide"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-between items-center">
            ...
            <div className="order-2 md:order-1">
              <div className="flex items-center gap-2 mb-3">
                <Lock className="w-4 h-4 text-mono-light-grey" />
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <a
                    ...
                    className="w-10 h-10 border border-mono-dark-grey flex items-center justify-center text-[#CCCCCC] hover:bg-white hover:text-black hover:border-white transition-all"
      ...
      <div className="border-t border-mono-dark-grey">
        <div className="container mx-auto px-4 md:px-6 py-4">
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center text-xs text-mono-light-grey">
