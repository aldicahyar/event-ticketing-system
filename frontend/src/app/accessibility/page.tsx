...
import { ArrowRight, FileText, ChevronRight, Accessibility } from 'lucide-react';
import { Navbar } from '@/components/layout/Navbar';
import { Footer } from '@/components/ui/Footer';
import { ACCESSIBILITY_SECTIONS, ACCESSIBILITY_META, type AccessibilityBlock } from '@/lib/accessibility-data';
...
export default function AccessibilityPage() {
  ...
  const [activeSection, setActiveSection] = useState<string>(ACCESSIBILITY_SECTIONS[0].id);
  ...
  useEffect(() => {
    ...
    ACCESSIBILITY_SECTIONS.forEach((s) => {
    ...
    <div className="min-h-screen bg-black text-white font-mono selection:bg-white selection:text-black">
      ...
      >
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            Our commitment to making live events accessible to everyone, online and in person. How we
            conform to WCAG 2.1 AA and what to do if you encounter barriers.
          </motion.p>
          <motion.div
            variants={staggerItem}
          ...
          >
            <span className="flex items-center gap-2 text-mono-light-grey uppercase tracking-widest">
              ...
              Last Updated: <span className="text-white font-bold">{ACCESSIBILITY_META.lastUpdated}</span>
            </span>
            <span className="flex items-center gap-2 text-mono-light-grey uppercase tracking-widest">
              Effective Date: <span className="text-white font-bold">{ACCESSIBILITY_META.effectiveDate}</span>
            </span>
            <span className="flex items-center gap-2 text-mono-light-grey uppercase tracking-widest">
              <Accessibility className="w-3 h-3" aria-hidden="true" />
              <span className="text-white font-bold">WCAG 2.1 AA</span>
            </span>
          </motion.div>
        </div>
      </motion.section>
      ...
      {/* Body: TOC + Content */}
      <section className="py-10 md:py-14" aria-label="Accessibility Statement content">
        <div className="container mx-auto px-4 md:px-6">
              ...
              <div className="max-w-3xl">
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      {section.body.map((block, i) => (
                        <AccessibilityBlockRenderer key={i} block={block} />
                      ))}
                    </div>
                  </motion.article>
                ))}
                ...
                >
                  ...
                  <p className="text-sm text-[#CCCCCC] mb-4">
                    Let us know and we will work to fix it. If you need help purchasing a ticket, contact
                    us immediately for an alternative method.
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      Report an Issue <ArrowRight className="w-3 h-3" aria-hidden="true" />
                    </Link>
                    <Link
                      href="/help"
...
function AccessibilityBlockRenderer({ block }: { block: AccessibilityBlock }) {
  switch (block.type) {
    case 'paragraph':
