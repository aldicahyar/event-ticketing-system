...
const booking = {
  ...
  event: {
    ...
    status: 'PUBLISHED',
    organizer_id: 'org-1',
    start_date_time: new Date('2026-08-10'),
  },
...
describe('RefundsService', () => {
  ...
  beforeEach(async () => {
    repository = {
      ...
      finalizeRefund: jest.fn().mockResolvedValue({ ...refund, status: RefundStatus.COMPLETED }),
  ...
  it('creates a refund request with server-calculated amount', async () => {
    ...
    expect(repository.create).toHaveBeenCalledWith(
      expect.objectContaining({ amount: 400000, percentage: 50 }),
    );
  ...
  it('prevents an organizer from processing another organizer event refund', async () => {
    repository.findById.mockImplementation((_id, organizerId) =>
      Promise.resolve(organizerId === 'org-1' ? refund : null),
    );

    await expect(
      service.approve('refund-1', { id: 'org-2', role: 'ORGANIZER' }, 'Approved after review'),
  ...
  it('rejects illegal terminal state transition', async () => {
    ...
    await expect(
      service.reject('refund-1', { id: 'admin-1', role: 'ADMIN' }, 'Not eligible'),
    ).rejects.toBeInstanceOf(ConflictException);
  });

  it('validates policy update has at least one field', async () => {
    await expect(service.updatePolicy('TIER_1_7D', {}, 'admin-1')).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });

  it('audits old/new values and invalidates cache after policy update', async () => {
    repository.findPolicy.mockResolvedValue({
      id: 'policy-1',
      rule_code: 'TIER_1_7D',
      percentage: 50,
      is_active: true,
    ...
    expect(repository.createPolicyAudit).toHaveBeenCalledWith(
      expect.objectContaining({ old_value: '50', new_value: '75', changed_by: 'admin-1' }),
    );
  });
});
