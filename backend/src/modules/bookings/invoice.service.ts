...
export class InvoiceService {
  ...
  private render(booking: BookingForInvoice, totals: InvoiceTotals): Promise<Buffer> {
    ...
    if (issuerAddress)
      doc.text(issuerAddress, PAGE_MARGIN, doc.y, { width: metaLeft - PAGE_MARGIN - 20 });
    ...
    doc
      ...
      .fontSize(8.5)
      .fillColor(C.muted)
      .text(formatJakartaDate(booking.event.start_date_time), metaLeft, doc.y, {
    ...
    for (let i = 0; i < booking.seats.length; i += 1) {
      ...
      doc.text(`${booking.seats[i].row}-${booking.seats[i].number}`, cols[0], y, {
        lineBreak: false,
      });
      doc.text(booking.seats[i].type, cols[1], y, { width: 120, lineBreak: false });
      doc
        .font('Courier')
        .text(money(price), cols[2], y, { width: 100, align: 'right', lineBreak: false });
    ...
    doc
      .moveTo(totalLeft + 16, ruleY)
      .lineTo(right - 16, ruleY)
      .strokeColor(C.white)
      .lineWidth(0.6)
      .stroke();
    doc
      .moveTo(totalLeft + 16, ruleY + 2)
      .lineTo(right - 16, ruleY + 2)
      .strokeColor(C.white)
      .lineWidth(0.6)
      .stroke();
    ...
    if (booking.payment) {
      const paidAt = booking.payment.paid_at
        ? formatJakartaDate(booking.payment.paid_at)
        : 'Not paid';
      doc.text(
        `Method: ${booking.payment.provider}    Status: ${paymentLabel(booking.payment.status)}    Paid: ${paidAt}`,
      );
      if (booking.payment.provider_tx_id)
        doc
          .font('Courier')
          .fillColor(C.muted)
          .text(`Transaction reference: ${booking.payment.provider_tx_id}`);
    } else {
